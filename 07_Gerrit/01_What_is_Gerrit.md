...
```mermaid
flowchart LR
  subgraph Gerrit["Gerrit"]
    G1["리뷰 방식: 모든 변경이 리뷰 필수"]
    G2["병합 방식: 리뷰어 승인 후 버튼 클릭"]
    G3["Push 방식: refs/for/<branch>"]
    G4["변경 이력: Patch Set 관리"]
    G5["대상: 대규모 팀, 엄격한 리뷰"]
    G6["설치: 자체 호스팅 필요"]
  end
...
```mermaid
flowchart TB
  Change["📦 Change<br/>하나의 PR 단위 (커밋 하나당 하나의 Change)"]
  ChangeID["🆔 Change-ID<br/>고유 ID (커밋 메시지에 포함)"]
  PatchSet["📋 Patch Set<br/>Change의 버전 (PS1, PS2, ...)"]
  Reviewer["👤 Reviewer<br/>코드 리뷰를 수행하는 사람"]
  Submit["✅ Submit<br/>리뷰 완료 후 변경을 병합"]
  Verified["✅ +2 Verified<br/>CI/테스트 통과 확인"]
  CodeReview["👍 +2 Code-Review<br/>코드 리뷰 승인"]
  Label["🏷️ Label<br/>리뷰 점수 (-2, -1, 0, +1, +2)"]
```
