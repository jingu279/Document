...
### 6.4.1 단기 메모리 (Short-term Memory)
...
## 6.6 Agent 설계 실전 Skills

### 6.6.1 Skill 1: Tool 설계 스킬

Agent 시스템에서 Tool은 외부 세계와 소통하는 유일한 창구이므로, Tool의 설계 품질이 Agent의 전체 성능을 결정합니다. 좋은 Tool 설계의 핵심 원칙은 "Agent가 Tool을 오용할 여지를 없애는 것"입니다. Tool의 이름은 그 기능을 직관적으로 나타내야 하며, description에는 "언제", "왜" 이 Tool을 사용해야 하는지를 구체적으로 명시해야 합니다. 파라미터 설계에서는 타입, 제약 조건, 기본값을 철저히 정의하여 Agent가 잘못된 인자를 전달할 가능성을 최소화해야 합니다. 특히 enum 타입을 활용하여 허용 가능한 값을 명시적으로 제한하거나, minimum/maximum 제약을 통해 입력 범위를 한정하는 것이 효과적입니다. 또한 Tool의 출력은 항상 구조화된 형태(JSON)로 반환하여 Agent가 다음 의사결정에 활용하기 쉽도록 설계해야 하며, 오류 발생 시에는 오류 코드와 해결 방안을 함께 제공하여 Agent의 자기 복구를 지원해야 합니다.

다음 표는 좋은 Tool 설계와 나쁜 Tool 설계를 구체적인 사례를 통해 비교한 것입니다.

| 설계 요소 | 좋은 Tool 설계 | 나쁜 Tool 설계 |
|---------|---------------|---------------|
| **Tool 이름** | `search_product_by_name(name: string)` — 목적과 동작이 명확함 | `search(q)` — 너무 추상적이고 모호함 |
| **설명(description)** | "제품명으로 재고를 검색합니다. 정확한 제품명을 알 때 사용하고, 부분 검색은 지원하지 않습니다." — 사용 조건과 제약을 명시 | "제품 검색 함수" — 언제 써야 하는지 알 수 없음 |
| **파라미터 타입** | `status: enum["pending", "shipped", "delivered"]` — 허용 값을 제한하여 오류 방지 | `status: string` — 잘못된 값 전달 가능 |
| **입력 제약** | `price: number (minimum: 0, maximum: 1000000)` — 범위를 명확히 제한 | `price: number` — 음수나 비현실적 값 허용 |
| **오류 반환** | `{"error": "NOT_FOUND", "message": "제품 ID 12345가 존재하지 않습니다", "suggestion": "목록 조회 API를 먼저 호출하십시오"}` — 원인과 해결책 제시 | `{"error": true}` 또는 빈 응답 — 원인 파악 불가 |
| **출력 형식** | `{"products": [{"id": "P001", "name": "노트북", "price": 1500000}], "total_count": 1}` — 구조화된 형식으로 Agent가 바로 활용 | "검색 결과: 노트북 1개 찾음" — 자연어라 파싱 어려움 |

### 6.6.2 Skill 2: Agent 판단력 향상 스킬

Agent의 판단력을 높이기 위해서는 단순히 모델에 의존하는 것이 아니라, 프롬프트 설계를 통해 Agent가 더 체계적으로 사고하고 결정을 내리도록 유도해야 합니다. 가장 중요한 원칙은 Agent가 명확성의 정도를 스스로 평가하여, 정보가 부족할 때는 추측하지 않고 적극적으로 질문하도록 설계하는 것입니다. 예를 들어 "사용자의 요청을 분석하여 필요한 정보가 모두 있는지 판단하고, 부족한 정보가 있다면 구체적으로 질문하십시오"라는 지침을 프롬프트에 포함시키면, Agent가 모호한 요청을 받았을 때 함부로 추측하지 않고 확인하는 습관을 가지게 됩니다. 또한 Agent가 의사결정을 내릴 때마다 "현재까지의 증거 요약 → 가능한 선택지 → 각 선택지의 장단점 → 최종 결정"이라는 구조화된 추론 과정을 거치도록 유도하면 판단의 일관성과 정확성이 크게 향상됩니다. 이와 함께 중요한 결정(예: 금전적 영향이 있는 작업, 사용자 데이터 수정 등)에는 반드시 휴먼 컨펌을 받도록 설계하고, 그 외의 일상적인 결정은 Agent가 자율적으로 처리하도록 구분하는 것이 효율적입니다. Agent의 판단력을 지속적으로 개선하기 위해서는 실제 운영 환경에서 Agent의 의사결정 로그를 수집하고, 잘못된 판단 사례를 분석하여 프롬프트에 반영하는 피드백 루프를 구축하는 것도 필수적입니다.

### 6.6.3 Skill 3: 오류 복구 스킬

Agent 시스템은 외부 API, 데이터베이스, LLM 등 여러 의존성을 가지므로 오류 발생이 불가피합니다. 따라서 오류를 처음부터 방지하는 것보다, 오류가 발생했을 때 Agent가 스스로 복구할 수 있는 탄력적인 설계가 더 중요합니다. 핵심 전략은 재시도(Retry), 대체 경로(Fallback), 인간 전달(Human Handoff)의 세 가지 계층으로 구성하는 것입니다. 재시도는 일시적인 네트워크 오류나 타임아웃과 같이 동일한 요청을 다시 보내면 해결될 가능성이 있는 오류에 적용합니다. 대체 경로는 기본 Tool이 실패했을 때 다른 방식으로 동일한 목적을 달성하는 방법입니다. 예를 들어 데이터베이스 직접 조회가 실패하면 캐시된 데이터를 사용하거나, 검색 API가 실패하면 로컬 인덱스를 검색하는 방식입니다. 인간 전달은 Agent가 자체적으로 해결할 수 없는 심각한 오류나, 법적/금전적 영향이 있는 결정이 필요한 경우 사람에게 제어권을 넘기는 최후의 안전장치입니다. 이러한 세 계층의 오류 복구 전략을 Agent 프롬프트에 명시적으로 지침으로 포함시키면, Agent는 오류 상황에서도 침착하게 대응하고 사용자에게 투명한 정보를 제공할 수 있습니다.

다음 Mermaid 다이어그램은 Agent의 오류 복구 의사결정 과정을 단계별로 나타냅니다.

```mermaid
flowchart TD
    A[도구 호출 실행] --> B{호출 성공?}
    B -->|성공| C[결과 반환 및 다음 단계 진행]
    B -->|실패| D{재시도 가능?}
    D -->|예| E[최대 3회 재시도]
    E --> B
    D -->|아니오| F{대체 경로 존재?}
    ...
    I -->|예| J[휴먼 핸드오프
    오류 정보와 함께 상세 전달]
    I -->|아니오| K["우아한 실패<br/>현재 처리할 수 없는 요청입니다<br/>+ 대체 방안 제안"]
    J --> L[담당자 검토 및 처리]
    K --> M[사용자에게 안내]
    L --> C
