...
## 실전 프로젝트: 회사 내부 문서 기반 Q&A 시스템 설계
...
### 5.5.3 쿼리 변환(Query Transformation)
...
문서 A(API 레퍼런스)에는 Document-Based 청킹 전략이 가장 적합합니다. API 레퍼런스는 함수명, 파라미터, 반환값이 엔드포인트나 함수 단위로 체계적으로 정리되어 있어, 원본 문서의 구조적 단위를 그대로 청크로 사용하는 것이 검색 정확도를 극대화합니다. 각 함수나 엔드포인트가 하나의 완결된 의미 단위를 이루므로, 이 구조를 유지하는 것이 관련성 높은 검색 결과를 보장합니다. 문서 B(고객 문의 내역)에는 Fixed Size 청킹이 적합합니다. 평균 50자의 짧은 비정형 텍스트가 5만 건이나 되므로, 각 문의를 개별 청크로 처리하고 메타데이터(문의 유형, 날짜, 처리 상태)를 함께 저장하여 필터링 검색이 가능하도록 해야 합니다. 문의 하나하나가 독립적인 의미 단위이므로 굳이 크게 묶을 필요가 없습니다. 문서 C(정책 백서)에는 Semantic 청킹이 가장 적합합니다. 백서는 서론, 본론, 결론의 논리적 흐름을 가지며 각 섹션이 특정 주제를 다루므로, 의미적 완결성을 기준으로 분할하면 사용자 질문에 가장 관련성 높은 단락을 정확히 검색할 수 있습니다. Recursive Character 방식도 고려할 수 있지만, 정책 문서는 주제 간 경계가 명확하므로 Semantic 방식이 더 높은 검색 품질을 제공합니다.

**문제 2 정답 및 해설:**

검색 결과에서 두 가지 문제점이 발견됩니다. 첫째, 1위 문서가 Fixed Size 500자 청킹으로 인해 문장 중간에서 분할되어 컨텍스트가 손실되었습니다. 둘째, 3위 문서인 "Vector DB 선택 가이드"는 환각 현상과 무관한 문서로 검색되어 관련성 필터링이 부족함을 보여줍니다. 가장 시급한 개선은 청킹 전략의 변경입니다. Fixed Size 방식을 Recursive Character 방식으로 전환하여 문장과 단락 단위로 자연스럽게 분할하면 1위 문서의 컨텍스트 손실 문제를 해결할 수 있습니다. 다음으로 쿼리 변환을 적용해야 합니다. 사용자의 질문 "RAG 시스템에서 환각 현상을 줄이는 방법"을 "RAG 시스템에서 LLM 환각 현상을 완화하기 위한 검색 품질 최적화, 프롬프트 엔지니어링, 청킹 전략"과 같이 더 구체적인 검색어로 확장하면 관련성 높은 문서가 상위에 랭크될 가능성이 높아집니다. 마지막으로 하이브리드 검색을 도입하여 벡터 검색과 키워드 검색을 병행하면, "Vector DB 선택 가이드"와 같은 무관한 문서가 검색 결과에서 제외되거나 순위가 크게 하락할 것입니다. 이러한 세 가지 개선을 순차적으로 적용하면 검색 정확도와 재현율이 모두 향상됩니다.

**문제 3 정답 및 해설:**

법률 문서 검색에서는 키워드 검색에 약간 더 높은 가중치를 부여하는 비대칭 RRF 가중치가 효과적입니다. 법률 도메인에서는 "계약 위반", "손해배상", "소멸시효"와 같은 정확한 법률 용어의 매칭이 매우 중요하며, 약간의 의미적 차이로 다른 법리가 적용될 수 있기 때문입니다. 구체적으로는 키워드 검색에 1.2의 가중치를, 벡터 검색에 0.8의 가중치를 적용하는 방식을 제안합니다. 이는 RRF 점수 계산 시 각 검색 방식의 순위 역수에 가중치를 곱하여 최종 점수를 산출하는 방식입니다. 예를 들어 특정 법 조문이 키워드 검색에서 1위, 벡터 검색에서 5위를 기록했다면, 최종 점수는 (1.2 × 1/1) + (0.8 × 1/5) = 1.36이 되어 관련 법 조문이 상위에 랭크됩니다. 그러나 완전히 키워드 검색에 의존하면 유사하지만 다른 표현을 사용한 관련 판례를 놓칠 수 있으므로, 벡터 검색의 의미적 유사성 점수도 일정 부분 반영하는 균형이 필요합니다. 실제 운영 환경에서는 법률 전문가의 피드백을 바탕으로 A/B 테스트를 통해 최적의 가중치를 미세 조정하는 과정이 추가되어야 합니다.
