...
flowchart TB
  subgraph System[시스템 Python]
    SYS["Python 3.12<br/>시스템 전체 하나"] --> SysExit
  end
  SysExit["→"]
...
flowchart TB
  ...
  subgraph ML[머신러닝]
    MLIn --> SK["Scikit-learn<br/>ML 알고리즘"]
  end
  MLIn["←"]

  subgraph DL[딥러닝]
    DLIn --> PT["PyTorch<br/>연구용 DL"]
    DLIn --> TF["TensorFlow<br/>산업용 DL"]
    PT --> DLOut
    TF --> DLOut
  end
  DLIn["←"]
  ...
  subgraph NLP[NLP & LLM]
    NLPIn --> HF["Hugging Face<br/>Transformers"]
    HF --> LC["LangChain<br/>LLM 앱"]
  end
  NLPIn["←"]
