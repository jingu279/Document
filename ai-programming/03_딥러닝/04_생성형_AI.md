...
> - AI Harness의 개념과 LLM/Agent 평가 방법을 이해합니다.
...
### 12.10.4 LLM Evaluation Harness
...
11. **AI Harness**가 왜 중요한가요? Harness가 없을 때 발생할 수 있는 문제 3가지를 설명하세요.

12. API 토큰을 안전하게 관리하는 방법을 설명하고, 절대 해서는 안 되는 행동을 2가지 이상 쓰세요.
...
- **주관적 평가:** "더 좋아진 것 같아"라는 직관에 의존하게 됨. Harness는 객관적 메트릭 제공
- **재현 불가:** 지난주 모델과 오늘 모델을 동일한 조건에서 비교 불가. Harness는 자동화된 반복 테스트로 재현성 보장
- **회귀 미발견:** 새로운 기능 추가 시 기존 성능이 하락해도 모름. CI/CD 파이프라인에서 자동 탐지
- **디버깅 어려움:** 실패 지점을 특정하기 어려움. 단계별 로깅 및 검증으로 정확한 실패 지점 파악
...
- **절대 하면 안 되는 행동:**
  ...
  4. 프론트엔드 코드에 API 키 포함 (누구나 열람 가능)</details>
