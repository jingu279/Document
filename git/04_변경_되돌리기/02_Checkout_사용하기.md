...
Git 2.23 이후 `git checkout`의 역할은 `git switch`와 `git restore` 두 명령어로 분리되었습니다. `git switch`는 브랜치 간 전환 기능만 담당하며, `git switch <브랜치명>`으로 사용합니다. `git restore`는 파일 복원 기능을 담당하여, `git restore <파일명>`으로 작업 디렉토리의 파일을 되돌리거나, `git restore --staged <파일명>`으로 스테이징 영역에서 파일을 제거합니다. 이렇게 분리된 이유는 기존 `git checkout`이 브랜치 전환과 파일 복원이라는 전혀 다른 두 가지 기능을 동시에 제공하여 혼란을 야기했기 때문입니다. 각 명령어가 하나의 책임만 가지도록 분리함으로써 Git 사용자의 학습 곡선을 낮추고 명령어의 의도를 명확히 할 수 있습니다.

**문제 2 정답 및 해설:**

Detached HEAD 상태는 HEAD가 특정 브랜치가 아닌 특정 커밋을 직접 가리키고 있는 상태를 말합니다. 일반적으로 HEAD는 현재 브랜치를 가리키고, 브랜치는 다시 특정 커밋을 가리키는 간접 참조 구조를 가집니다. 그러나 Detached HEAD 상태에서는 이러한 간접 참조 없이 HEAD가 곧바로 커밋을 가리킵니다. 이 상태에서 새로운 커밋을 만들면 커밋 자체는 성공적으로 생성되지만, 이 커밋을 가리키는 브랜치가 없습니다. 따라서 다른 브랜치로 전환하면 이 커밋에 접근할 수 없게 되어 사라질 위험이 있습니다. 이 커밋을 영구히 보존하려면 `git switch -c <새브랜치명>`으로 새 브랜치를 생성해야 합니다. Detached HEAD는 주로 과거 특정 커밋을 실험적으로 확인할 때 유용하지만, 장기적인 작업은 피해야 합니다.

**문제 3 정답 및 해설:**

작업 디렉토리의 파일을 마지막 커밋 상태로 되돌리려면 `git restore <파일명>` 명령어를 사용합니다. 이 명령어는 지정된 파일의 변경 사항을 취소하고, 가장 최근 커밋의 상태로 파일을 복원합니다. 예를 들어 `README.md` 파일을 수정했지만 원래 상태로 되돌리고 싶다면 `git restore README.md`를 실행합니다. 주의할 점은 이 명령어는 작업 디렉토리의 변경 사항을 되돌릴 수 없게 삭제하므로, 실행 전에 정말 되돌려도 되는 내용인지 반드시 확인해야 합니다. Git 2.23 이전에는 `git checkout -- <파일명>`을 사용했으나, 현재는 `git restore`가 권장되는 방식입니다.

**문제 4 정답 및 해설:**

`git restore --staged <파일명>`은 스테이징 영역에서만 파일을 제거하여 Staged 상태를 해제하지만, 작업 디렉토리의 파일 내용은 수정하지 않습니다. 즉, `git add`를 취소하는 효과를 냅니다. 반면 `git restore --worktree <파일명>`은 작업 디렉토리의 파일을 마지막 커밋 상태로 되돌리지만, 스테이징 영역에는 영향을 주지 않습니다. 두 옵션을 함께 사용하면 `git restore --staged --worktree <파일명>`이 되며, 이는 스테이징 영역과 작업 디렉토리를 모두 마지막 커밋 상태로 초기화합니다. 이 효과는 기존의 `git checkout -- <파일명>`과 유사하지만, `git restore`는 더 명시적이고 안전한 방식으로 각 영역을 개별 제어할 수 있습니다.

**문제 5 정답 및 해설:**

Detached HEAD 상태에서 만든 실험적인 커밋을 영구히 보존하려면 `git switch -c <새로운 브랜치명>` 명령어를 사용하여 현재 위치에 새 브랜치를 생성해야 합니다. 이 명령어는 현재 HEAD가 가리키는 커밋을 기준으로 새 브랜치를 만들고, 동시에 해당 브랜치로 전환까지 수행합니다. 예를 들어 `git switch -c experiment/feature-x`를 실행하면, Detached HEAD 상태에서 만든 모든 커밋이 `experiment/feature-x` 브랜치에 포함되어 영구히 보존됩니다. 새 브랜치를 만들지 않고 다른 브랜치로 전환하면, 이 커밋들은 더 이상 참조할 방법이 없어 Git의 가비지 컬렉션 대상이 되어 사라질 수 있습니다. 따라서 Detached HEAD 상태에서 의미 있는 작업을 했다면 반드시 브랜치를 생성하는 것을 잊지 말아야 합니다.
