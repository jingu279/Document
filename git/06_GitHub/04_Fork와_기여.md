...
Fork는 다른 사람의 원격 저장소를 자신의 GitHub 계정으로 복사하는 기능입니다. Fork 후에는 세 가지 저장소가 관계를 맺게 됩니다. upstream은 원본 저장소(예: `original/project`)로, Fork의 대상이 되는 원천 저장소입니다. origin은 Fork한 자신의 저장소(예: `myaccount/project`)로, upstream을 내 계정으로 복사한 저장소입니다. 로컬 저장소는 자신의 컴퓨터에 있는 저장소로, 일반적으로 `origin`을 원격 저장소로 등록하여 작업합니다. 개발자는 로컬에서 작업하고 `origin`으로 푸시한 후, Pull Request를 통해 upstream에 기여를 요청합니다. 또한 upstream의 최신 변경 사항을 로컬로 가져와서 동기화하는 작업도 정기적으로 수행해야 합니다.

**문제 2 정답 및 해설:**

Fork를 사용한 오픈소스 기여 워크플로우는 다음과 같은 8단계로 이루어집니다. 1단계: 기여할 오픈소스 저장소를 자신의 계정으로 Fork합니다(GitHub 웹사이트에서 Fork 버튼 클릭). 2단계: Fork한 저장소를 로컬에 클론합니다(`git clone https://github.com/내계정/프로젝트.git`). 3단계: upstream을 원격 저장소로 추가합니다(`git remote add upstream https://github.com/원본/프로젝트.git`). 4단계: upstream의 최신 변경 사항을 가져옵니다(`git fetch upstream && git merge upstream/main`). 5단계: 기능 개발을 위한 feature 브랜치를 생성합니다(`git switch -c feature/my-contribution`). 6단계: 변경 사항을 커밋하고 origin에 푸시합니다(`git push origin feature/my-contribution`). 7단계: GitHub에서 Fork한 저장소로 이동하여 "Compare & pull request" 버튼으로 PR을 생성합니다. 8단계: 리뷰 피드백을 반영하고 PR이 병합되면, feature 브랜치를 삭제하고 다시 4단계로 돌아가 최신 상태를 유지합니다.

**문제 3 정답 및 해설:**

Fork 전략과 브랜치 전략은 각각 장단점이 뚜렷하며 상황에 따라 선택해야 합니다. Fork 전략은 각 개발자가 원본 저장소에 직접 쓰기 권한 없이 독립적인 저장소에서 작업하는 방식입니다. 장점은 원본 저장소에 직접적인 영향을 주지 않아 안전하고, 외부 기여자도 쉽게 참여할 수 있습니다. 단점은 동기화와 관리가 복잡하고, 여러 Fork 저장소 간의 협업이 어렵습니다. 반면 브랜치 전략은 모든 팀원이 동일한 저장소에서 브랜치를 만들어 작업하는 방식입니다. 장점은 관리가 단순하고 동기화가 쉬우며, 팀 내 협업이 원활합니다. 단점은 저장소에 쓰기 권한이 필요하여 외부 기여자에게는 적합하지 않습니다. Fork 전략은 오픈소스 프로젝트나 외부 기여를 받는 경우에 적합하고, 브랜치 전략은 사내 팀 프로젝트나 폐쇄된 조직에서 적합합니다.

**문제 4 정답 및 해설:**

원본 저장소(upstream)와 Fork 저장소를 동기화할 때 rebase를 사용하는 이유는 히스토리를 깔끔하게 유지하기 위해서입니다. rebase는 로컬 커밋을 upstream의 최신 커밋 위에 재배치하여 선형적인 히스토리를 만듭니다. merge를 사용하면 병합 커밋이 생겨 히스토리가 복잡해지지만, rebase를 사용하면 마치 최신 upstream에서 바로 작업을 시작한 것처럼 깔끔한 히스토리가 유지됩니다. `--force-with-lease` 옵션은 rebase 후 원격 저장소에 푸시할 때 사용하는 안전장치입니다. 일반 `--force`는 원격 저장소의 상태를 무시하고 강제로 덮어쓰지만, `--force-with-lease`는 원격 저장소의 최신 상태를 확인하고 다른 사람이 이미 push한 커밋이 있다면 강제 푸시를 거부합니다. 이 옵션은 팀 협업 환경에서 다른 팀원의 작업을 실수로 덮어쓰는 위험을 방지해 줍니다.
