...
GitHub Flow의 5가지 핵심 원칙은 다음과 같습니다. 첫째, "main 브랜치는 항상 배포 가능한 상태 유지" — main 브랜치의 코드는 언제든지 프로덕션에 배포할 수 있을 정도로 안정적이어야 합니다. 이 원칙은 긴급 배포가 필요할 때 즉시 대응할 수 있게 해줍니다. 둘째, "새 기능이나 버그 수정은 feature 브랜치에서 작업" — 모든 변경 사항은 main에서 분리된 feature 브랜치에서 작업하여 main의 안정성을 보호합니다. 셋째, "feature 브랜치를 원격에 자주 푸시하여 백업" — 작업 내용을 정기적으로 원격에 백업하여 데이터 손실을 방지하고, 다른 팀원이 진행 상황을 확인할 수 있게 합니다. 넷째, "Pull Request로 코드 리뷰 요청" — 모든 변경 사항은 PR을 통해 리뷰를 받아야 하며, 이는 코드 품질을 보장하고 팀 내 지식을 공유합니다. 다섯째, "병합 후 즉시 배포 가능" — PR이 병합되면 main 브랜치가 갱신되고, CI/CD 파이프라인이 자동으로 배포를 수행하여 변경 사항이 신속히 사용자에게 전달됩니다.

**문제 2 정답 및 해설:**

GitHub Flow와 Git Flow의 주요 차이점은 브랜치 구성, 배포 주기, 복잡도에서 나타납니다. 브랜치 구성 측면에서 GitHub Flow는 main 브랜치와 feature 브랜치만 사용하는 단순한 구조인 반면, Git Flow는 main, develop, feature, release, hotfix의 5가지 브랜치를 사용하는 복잡한 구조입니다. 배포 주기 측면에서 GitHub Flow는 지속적 배포(CI/CD)를 전제로 수시로 배포하는 반면, Git Flow는 정해진 릴리스 주기에 따라 배포하는 방식에 적합합니다. 복잡도 측면에서 GitHub Flow는 규칙이 단순하여 소규모 팀이나 빠른 개발 사이클에 적합하고, Git Flow는 체계적인 프로세스가 필요한 대규모 팀이나 정기 릴리스가 있는 제품에 적합합니다. GitHub Flow는 "단순함이 최고"라는 철학을, Git Flow는 "체계적인 프로세스"를 중시합니다.

**문제 3 정답 및 해설:**

긴급 버그가 발생했을 때 GitHub Flow에서는 다음과 같이 대응합니다. 1단계: main 브랜치에서 `hotfix/critical-issue` 브랜치를 생성합니다. 2단계: 해당 브랜치에서 버그를 수정하고 커밋합니다. 3단계: 원격 저장소에 푸시하고 Pull Request를 생성합니다. 4단계: PR 제목에 "[Hotfix]" 접두사를 붙여 긴급 상황임을 표시하고, 리뷰어에게 긴급 리뷰를 요청합니다. 5단계: 리뷰가 완료되면 PR을 main 브랜치에 병합합니다. 6단계: CI/CD 파이프라인이 자동으로 main 브랜치를 프로덕션에 배포합니다. 7단계: 핫픽스가 배포된 후, 정식 수정 사항이 있다면 별도의 feature 브랜치에서 더 체계적으로 작업합니다. GitHub Flow에서는 모든 브랜치가 main에서 시작하므로, 긴급 상황에서도 복잡한 브랜치 전환 없이 빠르게 대응할 수 있습니다.

**문제 4 정답 및 해설:**

여러 개발자가 동시에 기능을 개발할 때 GitHub Flow에서 발생할 수 있는 충돌 상황은 두 가지입니다. 첫째, 두 개발자가 동일한 파일의 같은 부분을 수정하는 경우 병합 충돌이 발생합니다. 해결 방법은 로컬에서 `git merge main`을 실행하여 최신 main 브랜치의 변경 사항을 feature 브랜치에 주기적으로 병합함으로써 충돌을 조기에 발견하고 해결하는 것입니다. 둘째, 한 개발자가 PR을 병합한 후, 다른 개발자의 feature 브랜치가 outdated된 main 브랜치를 기반으로 하고 있어 push가 거부되는 상황입니다. 해결 방법은 feature 브랜치에서 `git rebase main` 또는 `git merge main`을 실행하여 최신 main 브랜치의 변경 사항을 반영한 후, `git push --force-with-lease`로 원격에 푸시하는 것입니다. GitHub Flow의 핵심은 feature 브랜치를 자주 main과 동기화하여 충돌을 작게 유지하고, PR을 작은 단위로 자주 생성하여 리뷰 부담을 줄이는 것입니다.
