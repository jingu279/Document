...
브랜치의 정체: 커밋을 가리키는 화살표
  ...
  │ └──────┘     └──────┘     └──────┘             │
  │                                                │
  │   "브랜치 = 35바이트짜리 파일에 커밋 해시만 적힌 것"  │
  │   새 브랜치 생성 = 0.008초 (단순히 새 포인터 생성)  │
  └────────────────────────────────────────────────┘
```
...
```
(1) 처음: main만 존재              (2) experiment 브랜치 생성
...
└──────┘                          └──────┘
  HEAD → main                        │
                                   experiment (HEAD는 여전히 main)

(3) experiment로 전환 (switch)    (4) experiment에서 커밋
...
│ C1   │                          │ C1   │ │ C2   │ ← HEAD
└──────┘                          └──────┘ └──────┘
  HEAD → experiment                 C2: 실험적 변경

(5) main으로 돌아가기               (6) main에서도 커밋
