...
```
    ...
    │  │ origin/  │ ◄──────── │         main (로컬 브랜치)           │ │
    │  │ main     │           │                                     │ │
    │  │ (원격추적)│           │  커밋: C1 ── C2 ── C3 ── C4 ── C5  │ │
    │  └──────────┘           └─────────────────────────────────────┘ │
    │                                                                 │
    │  pull = fetch + merge (원격 → 로컬, 자동 병합)                   │
    └─────────────────────────────────────────────────────────────────┘
```
...
| `git pull` | `git fetch` |
|---|---|
| 원격 변경 사항을 가져와서 자동으로 병합 | 원격 변경 사항을 가져오기만 함 |
| 바로 작업을 이어나갈 수 있음 | 변경 사항을 검토한 후 수동으로 병합해야 함 |
| 편리하지만 예상치 못한 충돌이 발생할 수 있음 | 더 안전하게 변경 사항을 관리할 수 있음 |
