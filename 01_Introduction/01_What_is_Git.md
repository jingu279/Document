...
전통적 방식 (파일명으로 관리):
  [index.html]────[index_v2.html]────[index_v3.html]────[index_최종.html]
                                                                   │
                                                                   ▼
                                                     "뭐가 최종 버전이지...?"

Git 방식 (스냅샷으로 관리):
  [커밋 a1b2c3d]────[커밋 d4e5f6f]────[커밋 g7h8i9j]────[커밋 k1l2m3n (HEAD)]
       │                  │                  │                  │
       ▼                  ▼                  ▼                  ▼
  "첫 번째 버전"    "로그인 버튼 추가"   "레이아웃 수정"    "푸터 추가"
                                                              │
                                                              ▼
                                              "git log로 언제든지 확인 가능!"
```
...
*   **분산형 (Distributed):** 중앙 서버에만 의존하는 것이 아니라, 모든 개발자가 프로젝트의 전체 이력(모든 파일과 모든 변경 사항)을 자신의 로컬 컴퓨터에 복사본으로 가지고 있습니다. 덕분에 인터넷 연결이 없어도 작업할 수 있으며, 중앙 서버에 문제가 발생해도 데이터 손실의 위험이 적습니다.
    ...
    **중앙집중식 vs 분산식 구조 비교:**
    ...
    중앙집중식 (SVN):              분산식 (Git):
     ...
     ▼         ▼         ▼       ▼          ▼          ▼
  ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐  ┌──────┐  ┌──────┐
  │개발자A│ │개발자B│ │개발자C│ │개발자A│  │개발자B│  │개발자C│
  │일부만 │ │일부만 │ │일부만 │ │전체  │  │전체  │  │전체  │
  │가짐   │ │가짐   │ │가짐   │ │이력! │  │이력! │  │이력! │
  └──────┘ └──────┘ └──────┘ └──────┘  └──────┘  └──────┘
    서버 없으면       서버가 죽어도
    작업 불가!        각자 계속 작업 가능!
